...
export default function SystemLogModal({
  ...
  const scanEngine =
    scan?.engine === 'candle_pattern' || scan?.decision?.engine === 'candle_pattern'
      ? 'Candle Brain'
      : scan?.engine === 'smc_vsa'
        ? 'Legacy'
        : scan?.engine === 'uvss'
        ...
        <div className="flex flex-col flex-1 min-h-0">
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
            ...
            <div className="bg-[#161A1E] border border-cyan-800/50 rounded-xl p-2">
              ...
              <StatusPill ok label="BIBLE V1" />
              <p className="text-[11px] text-gray-500 mt-2">Traps · 10 patterns · structure · 1:2 SL/TP</p>
            </div>
