const SECTIONS = [
  {
    ...
    items: [
      ...
      'Brain 5-step pipeline: (1) pattern detect → (2) trap scan → (3) candle-1 confirm on 1m/5m only; 15m/1h/1D fire on pattern close → (4) 10th-man ALLOW/VETO → (5) fire or skip.',
      ...
      '1m/5m: if pattern OF side score < 30, 10th-man picks the true trade (trap/stronger OF/stance) — do not skip. Other TFs still skip weak scores.',
      '1m/5m: Step3 = impulse then trail% pullback post-only maker. 15m/1h/1D skip extra confirm bars (pattern close is confirm), then same maker fill. Opposite color skips only before impulse lock. Miss / move ≥ profit / 10th-man VETO = skip (no taker chase). Exit market/taker.',
