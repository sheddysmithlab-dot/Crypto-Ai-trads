...
# ---------------------------------------------------------------------------
# Position-management helpers (exit only — does not change entry architecture)
# ---------------------------------------------------------------------------

# Opposite-of-position patterns that accelerate exit (side on pattern = trade direction they favor).
# LONG exits when bearish confirmation appears (pattern.side == SHORT).
# SHORT exits when bullish confirmation appears (pattern.side == LONG).
_LONG_EXIT_PATTERNS = {
    ("BUY_TRAP", "SHORT"),           # Buy Trap against LONG
    ("ABSORPTION", "SHORT"),         # Bearish absorption (buys absorbed)
    ("EXHAUSTION", "SHORT"),         # Buyer exhaustion
    ("FAKE_BREAKOUT", "SHORT"),      # Failed High
    ("REVERSAL_TRAP", "SHORT"),      # Bearish reversal trap
}
_SHORT_EXIT_PATTERNS = {
    ("SELL_TRAP", "LONG"),           # Sell Trap against SHORT
    ("ABSORPTION", "LONG"),          # Bullish absorption (sells absorbed)
    ("EXHAUSTION", "LONG"),          # Seller exhaustion
    ("FAKE_BREAKOUT", "LONG"),       # Failed Low
    ("REVERSAL_TRAP", "LONG"),       # Bullish reversal trap
}


def _iter_of_patterns(snapshot: Optional[dict]) -> List[dict]:
    if not snapshot:
        return []
    details = snapshot.get("details") if isinstance(snapshot.get("details"), dict) else {}
    out: List[dict] = []
    for key in ("patterns_1m", "patterns_5m"):
        pats = details.get(key) or snapshot.get(key) or []
        if isinstance(pats, list):
            out.extend(p for p in pats if isinstance(p, dict))
    for key in ("setup_1m", "setup_5m"):
        setup = details.get(key) or snapshot.get(key)
        if isinstance(setup, dict) and setup.get("name"):
            out.append(setup)
    # Top-level pattern name from evaluate_trap_orderflow
    top = snapshot.get("pattern")
    signal = snapshot.get("final_signal") or snapshot.get("signal")
    if top and top not in ("NONE", "BALANCED", "IMBALANCE", "RAW_IMBALANCE"):
        # Map signal LONG/SHORT → pattern side when possible
        side = "LONG" if signal in ("LONG", "BUY") else ("SHORT" if signal in ("SHORT", "SELL") else None)
        if side:
            out.append({"name": str(top).split("+")[0], "side": side, "reason": snapshot.get("primary_reason") or top})
    return out


def adverse_trap_exit_reason(position_side: str, of_snapshot: Optional[dict]) -> Optional[str]:
    """If confirmed opposite trap/absorption/exhaustion/failed H/L — return exit reason."""
    side = (position_side or "").upper()
    if side not in ("LONG", "SHORT"):
        return None
    targets = _LONG_EXIT_PATTERNS if side == "LONG" else _SHORT_EXIT_PATTERNS
    labels = {
        ("BUY_TRAP", "SHORT"): "Buy Trap",
        ("ABSORPTION", "SHORT"): "Bearish Absorption",
        ("EXHAUSTION", "SHORT"): "Buyer Exhaustion",
        ("FAKE_BREAKOUT", "SHORT"): "Failed High",
        ("REVERSAL_TRAP", "SHORT"): "bearish Reversal Trap",
        ("SELL_TRAP", "LONG"): "Sell Trap",
        ("ABSORPTION", "LONG"): "Bullish Absorption",
        ("EXHAUSTION", "LONG"): "Seller Exhaustion",
        ("FAKE_BREAKOUT", "LONG"): "Failed Low",
        ("REVERSAL_TRAP", "LONG"): "bullish Reversal Trap",
    }
    for p in _iter_of_patterns(of_snapshot):
        name = str(p.get("name") or "").upper()
        pside = str(p.get("side") or "").upper()
        key = (name, pside)
        if key in targets:
            nice = labels.get(key, f"{name}/{pside}")
            detail = p.get("reason") or ""
            return f"Opposite confirmation: {nice}" + (f" — {detail}" if detail else "")
    return None


def momentum_of_strength(position_side: str, of_snapshot: Optional[dict]) -> str:
    """Return 'strong' | 'weak' | 'unknown' for holding toward hard TP.

    Strong = 1M + 5M continue in trade direction with favorable volume/OF scores.
    Weak = contradiction, opposite score lead, or adverse bias/direction.
    """
    if not of_snapshot:
        return "unknown"
    side = (position_side or "").upper()
    details = of_snapshot.get("details") if isinstance(of_snapshot.get("details"), dict) else {}
    bias_5m = str(of_snapshot.get("bias_5m") or details.get("bias_5m") or "neutral").lower()
    dir_1m = str(details.get("dir_1m") or of_snapshot.get("dir_1m") or "neutral").lower()
    long_score = float(of_snapshot.get("long_score") or 0)
    short_score = float(of_snapshot.get("short_score") or 0)
    buy_p = float(of_snapshot.get("buy_pressure") or 0.5)
    sell_p = float(of_snapshot.get("sell_pressure") or 0.5)
    buy_v = float(of_snapshot.get("buy_volume_ratio") or 0.5)
    sell_v = float(of_snapshot.get("sell_volume_ratio") or 0.5)

    if side == "LONG":
        aligned = bias_5m == "bullish" and dir_1m == "bullish"
        score_ok = long_score >= short_score + 5 and long_score >= 55
        flow_ok = (buy_p >= 0.52 or buy_v >= 0.52) and long_score > short_score
        opposed = bias_5m == "bearish" or dir_1m == "bearish" or short_score >= long_score + 8
        if opposed or adverse_trap_exit_reason("LONG", of_snapshot):
            return "weak"
        if aligned and score_ok and flow_ok:
            return "strong"
        if (not aligned) or short_score > long_score or (buy_p < 0.48 and buy_v < 0.48):
            return "weak"
        return "unknown"

    if side == "SHORT":
        aligned = bias_5m == "bearish" and dir_1m == "bearish"
        score_ok = short_score >= long_score + 5 and short_score >= 55
        flow_ok = (sell_p >= 0.52 or sell_v >= 0.52) and short_score > long_score
        opposed = bias_5m == "bullish" or dir_1m == "bullish" or long_score >= short_score + 8
        if opposed or adverse_trap_exit_reason("SHORT", of_snapshot):
            return "weak"
        if aligned and score_ok and flow_ok:
            return "strong"
        if (not aligned) or long_score > short_score or (sell_p < 0.48 and sell_v < 0.48):
            return "weak"
        return "unknown"
...
def of_snapshot_from_detect(detect: Optional[dict]) -> Optional[dict]:
    """Normalize brain/detect payload into a compact OF snapshot for exit mgmt."""
    if not detect or not isinstance(detect, dict):
        return None
    of = detect.get("orderflow_trap")
    if isinstance(of, dict) and (of.get("long_score") is not None or of.get("details") or of.get("pattern")):
        return {
            "ts": time.time(),
            "pattern": of.get("pattern") or detect.get("trap_type") or detect.get("pattern"),
            "bias_5m": of.get("bias_5m"),
            "buy_pressure": of.get("buy_pressure"),
            "sell_pressure": of.get("sell_pressure"),
            "buy_volume_ratio": of.get("buy_volume_ratio"),
            "sell_volume_ratio": of.get("sell_volume_ratio"),
            "long_score": of.get("long_score"),
            "short_score": of.get("short_score"),
            "final_signal": of.get("final_signal") or of.get("signal"),
            "primary_reason": of.get("primary_reason") or of.get("line") or of.get("reason"),
            "details": of.get("details") or {},
            "line": of.get("line"),
        }
    # Fallback: minimal from detect fields
    return {
        "pattern": detect.get("trap_type") or detect.get("pattern"),
        "long_score": detect.get("long_score"),
        "short_score": detect.get("short_score"),
        "final_signal": "LONG" if detect.get("action") == "BUY" else (
            "SHORT" if detect.get("action") == "SELL" else None
        ),
        "details": {},
    }
