...
async def score_universe(
    ...
    for coin, bybit_symbol in (symbol_map or {}).items():
        pair = f"{coin}/USDT"
        move: dict = {}
        try:
    ...
    rows.sort(key=lambda r: (not r["passed"], -(r["avg_pct"] if r["avg_pct"] is not None else -1.0)))
    return rows
...
async def build_momentum_watchlist(
    *,
    ...
    thr = momentum_threshold_pct(engine_tf)
    scores = await score_universe(symbol_map, engine_tf)
