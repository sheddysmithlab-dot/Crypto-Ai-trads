...
_CONFIRM_SYSTEM = (
    ...
    "(overall ≥75%; 5m ≥85%; named traps ≥90%). Otherwise reply NO. "
...
def _ai_yes_thr_for_tf(timeframe_key: str, pattern: str | None = None) -> float:
    """AI YES floor: named traps ≥90, 5m ≥85, otherwise overall ≥75."""
...
def _build_confirm_user_prompt(
    *,
    ...
    lines = [
        ...
        f"(overall ≥75; 5m ≥85; named traps ≥90). Otherwise NO.",
        "",
        "ANALYZE (policy):",
...
def entry_pattern_profile(timeframe_key: str | None = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "description": (
            ...
            f"Order-flow conf floor: overall ≥75% / 5m ≥85% / named traps ≥90% "
...
def strategy_system_blurb() -> str:
    return (
        ...
        "5) If AI offline: strong OF setup (overall≥75 / 5m≥85 / trap≥90) or brain.py verdict as fallback.\n"
    )
