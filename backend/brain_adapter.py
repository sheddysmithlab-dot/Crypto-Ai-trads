...
_CONFIRM_SYSTEM = (
    ...
    "(1m≥85%, 5m≥70%, other≥65%). Otherwise reply NO. "
...
def _ai_yes_thr_for_tf(timeframe_key: str) -> float:
    """AI may only YES at/above these confidence floors (1m=85, 5m=70, else=65)."""
...
def _build_confirm_user_prompt(
    *,
    ...
    lines = [
        ...
        f"(1m=85, 5m=70, else=65). Otherwise NO.",
        "",
        "ANALYZE (policy):",
...
def entry_pattern_profile(timeframe_key: str | None = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "description": (
            ...
            f"(1m=85, 5m=70, else=65; AI YES only at/above floor). "
            f"{tf_cfg.note}"
        ),
