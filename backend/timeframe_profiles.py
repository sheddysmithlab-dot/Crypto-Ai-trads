...
# 1h / 1D: closed pattern bar is the confirm — extra same-TF bars are too slow.
SKIP_PATTERN_CONFIRM_TFS = frozenset({"1h", "1D"})
...
def skips_pattern_confirm(timeframe_key: str | None) -> bool:
    """1h / 1D fire on pattern close; 1m/5m/15m wait extra confirm bars."""
